...
export default function EditTemplatePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [productsRes, templateRes] = await Promise.all([
          fetch("/api/products"),
          fetch(`/api/process-templates/${params.id}`),
        ]);
        ...
        setForm({
          ...
          productId: template.productId ? String(template.productId) : "",
        });
        setSteps(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/process-templates/${params.id}`, {
        method: "PUT",
